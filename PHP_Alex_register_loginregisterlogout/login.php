...
/*Je vérifie dans le if suivant que fetch arrive a récupérer le mail rentré par l'utilisateur. Je stock aussi les résultat du fetch (donc la ligne tableau de l'utilisateur) dans la variable passUser*/
	...
	if ( $passUser = $checkMail ->fetch() ) {
		...
		if (password_verify( $_POST["password"], $passUser["password"] ) ){
			$_SESSION["userConnected"] = true;
			$_SESSION["user"] = $passUser;
